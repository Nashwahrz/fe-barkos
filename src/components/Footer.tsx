...
export default function Footer() {
  ...
  if (pathname.match(/^\/chat\/\d+\/\d+/) || pathname.startsWith('/admin') || pathname.startsWith('/auth')) {
    return null;
  }

  // Sellers & admins already have selling access — hide the "start selling" pitch for them.
  const canPitchSelling = !user || (user.role !== USER_ROLES.PENJUAL && user.role !== USER_ROLES.SUPER_ADMIN);
  const sellLink = !user ? '/auth/login?redirect=/seller/register' : '/seller/register';
  ...
  return (
    <footer className="footer-container" style={{
      ...
      {/* CTA band — only pitched to buyers / guests, not to existing sellers or admins */}
      {canPitchSelling && (
        <div style={{
        ...
        <div style={{
          ...
          <div>
            ...
            <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: '0.85rem' }}>
              ...
              {canPitchSelling && (
                <li><Link href={sellLink} style={linkStyle} onMouseEnter={hoverIn} onMouseLeave={hoverOut}>Mulai Jual</Link></li>
              )}
            </ul>
          </div>
